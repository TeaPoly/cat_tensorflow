...
class AudioSegment(object):
        ...
        if isinstance(file, str) and re.findall(r".seqbin_\d+$", file):
            ...
            if len(samples.shape) > 1:
                ...
                for i in np.arange(samples.shape[-1]):
                    audio_list.append(cls(samples[:, i], sample_rate))
                return audio_list
            # Single channel
            else:
    ...
    @classmethod
    def slice_from_file_random(cls, file, segment_length, rng=None):
        """Loads a small section of an audio without having to load
        the entire file into the memory which can be incredibly wasteful.
        :param file: Input audio filepath or file object.
        :type file: str|file
        :param segment_length: segment length in seconds.
        :type segment_length: float
        :return: AudioSegment instance of the specified slice of the input
                 audio file.
        :rtype: AudioSegment
        :raise ValueError: If segment_length is greater than file's duration in time.
        """
        ...
        if segment_length > duration:
            raise ValueError("Length of segment must not be greater "
                             "than original audio.")
        start_time = rng.uniform(0.0, duration - segment_length)

        return cls.slice_from_sndfile(sndfile, sample_rate, duration, start_time, start_time + segment_length)

    @classmethod
    def slice_pad_from_file_random(cls, file, segment_length, rng=None, sides='end'):
        """Loads a small section of an audio without having to load
        the entire file into the memory which can be incredibly wasteful.
        :param file: Input audio filepath or file object.
        :type file: str|file
        ...
        if segment_length > duration:
            if sides is None:
                raise ValueError("Length of segment must not be greater "
                                 "than original audio.")
            silence = np.zeros(int((segment_length-duration) * sample_rate))
            data = sndfile.read(frames=-1, dtype='float32')
            return cls(np.concatenate([data, silence]), sample_rate)
        else:
            start_time = rng.uniform(0.0, duration - segment_length)
            return cls.slice_from_sndfile(sndfile, sample_rate, duration, start_time, start_time + segment_length)
    ...
    def slice_from_file(cls, file, start=None, end=None):
        ...
        sample_rate = sndfile.samplerate
        duration = float(len(sndfile)) / sample_rate
        return cls.slice_from_sndfile(sndfile, sample_rate, duration, start, end)

    @classmethod
    def from_audio_segs(cls, segs_list):
        """create audio segment with a list of audio segments.

        :param segs_list: [audio_segment1, audio_segment2, ...]
        :returns: A audio segment object which hold the sumed up samples from audio segs.
        :rtype:

        """
        length = len(segs_list[0].samples)
        summed_samples = np.sum(
            [audio_seg.samples for audio_seg in segs_list], axis=0)
        if len(summed_samples) != length:
            raise ValueError("Summed Up samples length Mismatch %d != %d" % (
                length, len(summed_samples)))

        return cls(summed_samples,
                   segs_list[0].sample_rate)

    @classmethod
    def slice_from_sndfile(cls, sndfile, sample_rate, duration, start=None, end=None):
        """Loads a small section of an audio without having to load
        the entire file into the memory which can be incredibly wasteful.

        :param sndfile: Input SoundFile object.
        :type sndfile: SoundFile
    ...
    def superimpose(self, other):
        ...
        if len(self._samples) != len(other._samples):
            # print(len(self._samples), len(other._samples))
            raise ValueError("Segment lengths must match to add segments.")
        # if len(self._samples)<len(other._samples):
        #   other._samples = other._samples[:len(self._samples)]
        # else:
        #   self._samples = self._samples[:len(other._samples)]
    ...
    def speed_perturbation(self, lower=0.9, upper=1.1,
                           keep_length=True, res_type='kaiser_best', rng=None):
        """Sample a new speed rate from the given range and
            changes the speed of the given audio clip.

        Note that this is an in-place transformation.

        :param lower: min speed ratio.
        :param upper: max speed ratio.
        :param keep_length: whether to keep same length of original signal.
        :param res_type: kaiser_best or kaiser_best for re-samplerate.
        ...
        if rng is None:
            seed = None
            ratio = np.random.RandomState(seed).uniform(lower, upper)
        else:
            ratio = rng.uniform(lower, upper)

        samples = self.samples

        # Note1: resample requires the sampling-rate of input and output,
        #        but actually only the ratio is used.
        self._samples = librosa.resample(
            self.samples, ratio, 1, res_type=res_type)

        if keep_length:
            diff = abs(len(samples) - len(self._samples))
            if len(self._samples) > len(samples):
                # Truncate noise
                self._samples = self.samples[diff // 2:-((diff + 1) // 2)]
            elif len(self._samples) < len(samples):
                # Assume the time-axis is the first: (Time, Channel)
                pad_width = [(diff // 2, (diff + 1) // 2)] + \
                    [(0, 0) for _ in range(self.samples.ndim - 1)]
                self._samples = np.pad(self.samples, pad_width=pad_width, constant_values=0,
                                       mode='constant')
    ...
    def mixed_bandwidth_simulate(self, temporary_sample_rate=8000, filter='kaiser_best'):
        """Mixed bandwidth simulation for the audio.

        Note that this is an in-place transformation.

        :param temporary_sample_rate: Temporary sample rate.
        :type temporary_sample_rate: int
        :param filter: The resampling filter to use one of {'kaiser_best',
                       'kaiser_fast'}.
        :type filter: str
        """
        temporary_samples = resampy.resample(
            self.samples, self.sample_rate, temporary_sample_rate, filter=filter)

        self._samples = resampy.resample(
            temporary_samples, temporary_sample_rate, self.sample_rate, filter=filter)
    ...
    def add_noise(self,
        ...
        """
        # If current SNR is already less than target snr_dB, just skip
        # the noise adding operation.
        rng = random.Random() if rng is None else rng
    ...
    def convert_samples_from_float32(self, dtype='int16'):
        # export for integration with frontend
        return self._convert_samples_from_float32(self._samples, dtype)
